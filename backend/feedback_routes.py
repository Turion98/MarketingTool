...
class FeedbackPayload(BaseModel):
    sessionId: Optional[str] = Field(None, description="Frontend session / játék azonosító")
    pageId: Optional[str] = Field(None, description="Melyik oldalról küldték (pl. epilogue után)")
    email: Optional[EmailStr] = Field(None, description="Opcionális e-mail feliratkozás/kapcsolat")
...
def _read_last_lines(limit: int = 50, offset: int = 0) -> List[Dict[str, Any]]:
...
async def _maybe_send_email(record: Dict[str, Any]) -> None:
    """
...
@router.get("/feedback")
def list_feedback(
    secret: Optional[str] = Query(default=None, description="Olvasáshoz titkos kulcs szükséges"),

"""Phase 3c full-blueprint live apply.

Walks every AI page in an existing assembled story, calls the live
``AnthropicOnboardingClient.generate_reply_rules`` per page, applies the
returned reply_rules to the local copy, and lints the augmented story
end-to-end. The augmented story is saved to a sibling ``_phase3c.json``
file (the original is preserved).

Usage::

    python scripts/onboarding_phase3c_full_apply.py \
        --story data/onboarding/story_<ts>_phase3b.json \
        --blueprint data/onboarding/blueprint_<ts>.json
        [--output data/onboarding/story_<ts>_phase3c.json]
        [--locale-override hu|en]
        [--overwrite-existing]   # overwrite curated reply_rules

Required env: ``ANTHROPIC_API_KEY``.

Cost: ~14 AI calls × ~$0.015 ≈ $0.20, ~5-7 minutes.
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Full-blueprint live apply of Phase 3c reply_rules generator."
        )
    )
    ...
    parser.add_argument(
        ...
        help="Output story path. Default: <story>_phase3c.json",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Overwrite any existing reply_rules on steps (default: preserve).",
    )
    ...
    print(f"  locale: {locale}  vendor_policy: {vendor_policy}  vendor_name: {vendor_name!r}")
    ...
    _print_section("Phase 3c — generating reply_rules per node")
    ...
    print(f"  Steps populated:  {summary['total_steps_rules_applied']}")
    print()
    print("  Per-node breakdown:")
    for nid in summary["succeeded_node_ids"]:
        applied = summary["per_node_applied"].get(nid, 0)
        print(f"    {nid}: {applied} steps")
    ...
    # Sample 3-4 generated rules across the story for spot-check.
    _print_section("Sample reply_rules across the story")
    samples_shown = 0
    for pid in summary["succeeded_node_ids"]:
        ...
        for s in steps:
            if not isinstance(s, dict):
                continue
            rules = s.get("reply_rules")
            if not isinstance(rules, list) or not rules:
                continue
            print(f"  {pid}.{s.get('id')} ({len(rules)} rules):")
            for r in rules:
    ...
    # Final lint of the augmented story.
    rep = lint_full_story(
    ...
    # Save augmented story.
    out_path: Path = args.output or args.story.with_name(
        args.story.stem + "_phase3c.json"
    )
    out_path.write_text(

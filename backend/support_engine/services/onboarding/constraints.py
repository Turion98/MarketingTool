...
* `decision_engine.services.story_runtime.resolve_ai_node_routing` — routing AND-only,
  no negation, kétmenetes (if-rules → default), `goto` ∈ pages ∪ {"ask"}.
* `shared.story_lint.validate_*` — page / step / condition / routing /
  order_context_mapping struktúra.
* `decision_engine.services.order_context._apply_field_rules` — `when` ∈ {"truthy", "not_null"},
  vagy `when_value` / `when_any_value`; modifier-ek (`requires_field_not_null`,
  `requires_field_lt_today`).
* `decision_engine.services.order_context.OrderContext` — engedélyezett külső mező-pool
  (a `shared.story_lint.KNOWN_OCM_FIELDS` ezt tükrözi).
...
class RoutingConstraints(BaseModel):
    """A `decision_engine.services.story_runtime.resolve_ai_node_routing` által támogatott
...
class ConditionConstraints(BaseModel):
    ...
    Forrás: `shared.story_lint.validate_conditions` és `validate_steps`
...
# A `shared.story_lint.validate_steps` által elfogadott step típusok.
...
class StepConstraints(BaseModel):
    ...
    Forrás: `shared.story_lint.validate_steps` (~200 sor).
    """
...
# A `shared.story_lint.validate_page` által elfogadott page típusok.
...
class PageConstraints(BaseModel):
    ...
    Forrás: `shared.story_lint.validate_page`.
    """
...
# A `decision_engine.services.order_context._apply_field_rules` által támogatott `when` módok.
...
class OrderContextConstraints(BaseModel):
    ...
    A `decision_engine.services.order_context.OrderContext` a per-rendelés tényhalmaz; a
    `meta.order_context_mapping` deklaratívan írja le, melyik mező milyen
    érték esetén satisfy-jeli melyik kondíciót.

    A `known_external_fields` lista a `shared.story_lint.KNOWN_OCM_FIELDS`-ből
    ...
    known_external_fields: list[str] = Field(
        ...,
        description="Engedélyezett OrderContext mezők. Új mezőhöz a `decision_engine.services.order_context.OrderContext` BaseModel-t kell bővíteni + a `KNOWN_OCM_FIELDS` halmazt.",
    )
...
class MetaConstraints(BaseModel):
    ...
    Forrás: `shared.story_lint.validate_meta` és a `RUNTIME_INT_KEYS` /

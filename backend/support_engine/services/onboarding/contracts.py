...
* **Phase 2 — node generation.** Node-onként, sorban: a `GenerationContext`
  ...
  a `shared.story_lint.lint_single_node`-on. A retry-policy
...
if TYPE_CHECKING:
    from shared.story_lint import Report as _LintReport
...
class StructuralLintResult(BaseModel):
    """A `shared.story_lint.Report` Pydantic-projekciója (Phase 3a).
    ...
    def from_report(cls, report: "_LintReport") -> "StructuralLintResult":
        """Adapter: `shared.story_lint.Report` → `StructuralLintResult`.
